    ...
    <svg viewBox={`0 0 ${w} ${h}`} className="w-full h-full" preserveAspectRatio="none">
      <defs>
        ...
        <pattern id="diagonal" patternUnits="userSpaceOnUse" width="10" height="10" patternTransform="rotate(45)">
          <line x1="0" y1="0" x2="0" y2="10" stroke="#1B2735" strokeWidth="4" />
        </pattern>
      </defs>
...
export function AssetInfoModal({ asset, onClose, onTrade }: AssetInfoModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-16 px-4 bg-black/50">
      <div className="relative bg-[#1e2235] rounded-2xl border border-[#16202D] shadow-2xl w-full max-w-[820px] overflow-hidden">

        {/* Close button — outside card, top-left */}
        <button
          onClick={onClose}
          className="absolute -top-3 -left-3 w-8 h-8 bg-[#1e2235] border border-[#16202D] rounded-full flex items-center justify-center text-[#7E8DA2] hover:text-white transition-colors z-10 shadow-lg"
        >
          <X size={14} />
        </button>
        ...
        <div className="flex min-h-0">
          ...
          <div className="flex-1 p-5 flex flex-col gap-4 border-r border-[#16202D]">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-xs text-[#7E8DA2]">
                <span className="text-white font-medium">Abra agora</span>
                {' '}/ Fecha hoje às 20:59
              </div>
            </div>

            <div className="h-px bg-dashed border-t border-dashed border-[#16202D]" />

            {/* Price + session change + CTA */}
            <div className="flex items-center gap-8">
              <div>
                <div className="text-[11px] text-[#7E8DA2] mb-1">Preço agora</div>
                <div className="text-xl font-bold text-white font-mono">
                  {asset.price.toFixed(asset.price > 10 ? 3 : 5)}
                </div>
              </div>
              <div>
                <div className="text-[11px] text-[#7E8DA2] mb-1">Alteração de sessão</div>
            ...
            {/* Sentiment bar */}
            <div className="bg-[#101825] rounded-xl p-4">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <div className="text-sm font-bold text-white">Vender</div>
                  <div className="text-[10px] text-[#7E8DA2] mt-0.5">Sentimento dos<br />comerciantes</div>
                </div>
            ...
            <div className="grid grid-cols-4 gap-4">
              <div>
                <div className="text-[11px] text-[#7E8DA2] mb-1">Investimento mínimo</div>
                <div className="text-sm font-bold text-white">R$5</div>
              </div>
              <div>
                <div className="text-[11px] text-[#7E8DA2] mb-1">Lucro - 1 min</div>
                <div className="text-sm font-bold text-green-400">{asset.payout}%</div>
              </div>
              <div>
                <div className="text-[11px] text-[#7E8DA2] mb-1">Lucro - 5+ min</div>
                <div className="text-sm font-bold text-green-400">{asset.payout}%</div>
              </div>
              <div>
                <div className="text-[11px] text-[#7E8DA2] mb-1">Tempo de expiração</div>
            ...
            <div className="flex gap-3">
              ...
              <div className="flex flex-col gap-2 w-[180px] flex-shrink-0">
                {CHANGES.map((c) => (
                  <div key={c.label} className="bg-[#101825] rounded-lg px-3 py-2">
                    <div className="text-[10px] text-[#7E8DA2] leading-tight mb-1">{c.label}</div>
              ...
              {/* Mini chart */}
              <div className="flex-1 bg-[#101825] rounded-lg overflow-hidden h-[120px]">
                <MiniChart />
              </div>
            </div>

            {/* Bottom stats */}
            <div className="flex items-center gap-6 pt-1 border-t border-[#16202D]">
              {BOTTOM_STATS.map((s) => (
                <div key={s.label} className="flex items-center gap-1.5 text-xs">
                  <span className="text-[#7E8DA2]">{s.label}</span>
          ...
          <div className="w-[280px] flex-shrink-0 p-5">
            ...
            <div className="grid grid-cols-3 gap-1 mb-2">
              <div className="text-[10px] text-[#7E8DA2]">Encontro</div>
              <div className="text-[10px] text-[#7E8DA2]">Dia da semana</div>
              <div className="text-[10px] text-[#7E8DA2]">Horário de Negociação</div>
            </div>
            ...
            <div className="flex flex-col gap-0">
              {SCHEDULE.map((row, i) => (
                <div
                  key={row.date}
                  className={`grid grid-cols-3 gap-1 py-2 text-xs border-t border-[#16202D] ${
                    i === 0 ? 'text-white' : 'text-[#7E8DA2]'
                  }`}
                >
