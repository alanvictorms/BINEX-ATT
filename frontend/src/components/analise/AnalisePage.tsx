  ...
  return (
    <div className="relative flex items-center justify-center" style={{ width: size, height: size }}>
      <svg width={size} height={size} style={{ transform: 'rotate(-90deg)' }}>
        <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke="#16202D" strokeWidth={stroke} />
      ...
      <div className="absolute flex flex-col items-center justify-center">
        ...
        {percent !== undefined && <span className="text-[9px] text-[#7E8DA2] mt-0.5">{Math.round(percent * 100)}%</span>}
      </div>
    </div>
  )
}
    ...
    <div>
      ...
      <div className="flex gap-0.5 mb-1">
        {Array.from({ length: 8 }).map((_, i) => (
          <div key={i} className="h-1 w-3 rounded-sm bg-[#16202D]" />
        ))}
      </div>
      <div className="text-xs text-[#7E8DA2]">{label}</div>
    </div>
  )
}
    ...
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={H}>
      ...
      {yLabels.map((_, i) => (
        <line key={i} x1={padL} x2={W - padR} y1={padT + i * yStep} y2={padT + i * yStep}
          stroke="#16202D" strokeWidth="1" />
      ))}
      {/* Y labels */}
      {yLabels.map((l, i) => (
        <text key={i} x={padL - 6} y={padT + i * yStep + 4} textAnchor="end"
          className="fill-[#7E8DA2]" style={{ fontSize: 10 }}>{l}</text>
      ))}
      {/* Area fill */}
      <defs>
      ...
      {xLabels.map((l, i) => {
        ...
        return (
          <text key={i} x={xPos(idx)} y={H - 6} textAnchor="middle"
            className="fill-[#7E8DA2]" style={{ fontSize: 9 }}>{l}</text>
        )
      })}
    </svg>
  )
...
function BarChart({ bars }: { bars: { label: string; value: number }[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={H}>
      {yLabels.map((l, i) => (
        <g key={i}>
          <line x1={padL} x2={W - padR} y1={padT + i * yStep} y2={padT + i * yStep} stroke="#16202D" strokeWidth="1" />
          <text x={padL - 4} y={padT + i * yStep + 4} textAnchor="end" className="fill-[#7E8DA2]" style={{ fontSize: 9 }}>{l}</text>
        </g>
      ))}
      {bars.map((b, i) => {
        ...
        return (
          <g key={i}>
            ...
            <text x={x + barW / 2} y={H - 8} textAnchor="middle" className="fill-[#7E8DA2]" style={{ fontSize: 8 }}>{b.label}</text>
          </g>
        )
      })}
    </svg>
  ...
  return (
    <div className="flex items-center gap-6">
      <svg width={size} height={size}>
        <circle cx={cx} cy={cy} r={r} fill="none" stroke="#0E1620" strokeWidth={stroke + 2} />
        {segments.map((s, i) => {
        ...
        <circle cx={cx} cy={cy} r={r * 0.52} fill="#0A101A" />
      </svg>
      <div className="flex flex-col gap-1.5">
        {segments.map((s, i) => (
          <div key={i} className="flex items-center gap-2">
            <div className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ background: s.color }} />
            <span className="text-xs text-[#7E8DA2]">{s.label} <span className="text-white font-semibold">{s.pct}%</span></span>
          </div>
        ))}
      </div>
    </div>
...
export function AnalisePage() {
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#0A101A] min-h-0 overflow-hidden">
      ...
      {/* User info bar — overflow horizontal em mobile pra acomodar todos os dados */}
      <div className="flex items-center gap-3 sm:gap-6 px-3 sm:px-6 py-3 border-b border-[#16202D] bg-[#0E1620] flex-shrink-0 overflow-x-auto">
        {/* Avatar + info */}
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-1 text-xs text-[#7E8DA2]">
              ID: 10000001
        ...
        <div className="w-px h-8 bg-[#16202D]" />

        {/* Location */}
        <div>
          <div className="text-[10px] text-[#7E8DA2] mb-0.5">Localização</div>
        ...
        {/* Na conta */}
        <div>
          <div className="text-[10px] text-[#7E8DA2] mb-0.5">Na conta</div>
        ...
        <div className="flex items-center gap-3">
          <div>
            <div className="text-[10px] text-[#7E8DA2] mb-0.5">Na demonstração</div>
            <div className="text-sm font-semibold text-white">R$45.411,50</div>
          </div>
          <button onClick={() => setShowDemo(!showDemo)} className="text-[#7E8DA2] hover:text-white transition-colors">
            <Eye size={16} />
          </button>
        </div>
        ...
        {/* Period selector */}
        <button className="flex items-center gap-2 px-4 py-2 rounded-lg border border-[#16202D] bg-[#0E1620] text-sm text-white hover:border-white/30 transition-colors">
          Mês <ChevronDown size={14} className="text-[#7E8DA2]" />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="flex flex-col lg:flex-row min-h-full">
          ...
          <div className="w-full lg:w-[520px] lg:flex-shrink-0 border-b lg:border-b-0 lg:border-r border-[#16202D] px-4 sm:px-6 py-4 sm:py-5 flex flex-col gap-4 sm:gap-6">
            ...
            <div>
              ...
              {/* Row 1 */}
              <div className="grid grid-cols-3 gap-4 pb-4 border-b border-[#16202D]">
                <div className="flex flex-col items-start gap-2">
                  <Ring value={422} max={422} size={68} />
                  <span className="text-xs text-[#7E8DA2]">Contagem de negociações</span>
                </div>
                <div className="pt-1">
                ...
                <div className="flex flex-col items-start gap-2">
                  ...
                  <span className="text-xs text-[#7E8DA2]">Negociações lucrativas</span>
                </div>
              </div>

              {/* Row 2 */}
              <div className="grid grid-cols-3 gap-4 py-4 border-b border-[#16202D]">
              ...
              <div className="flex items-center gap-3 mt-4">
                ...
                <div className="flex gap-3 text-[10px] text-[#7E8DA2]">
                  <span>-1K-0</span>
                  <span>0-1K</span>
                  <span>+1K</span>
                </div>
          ...
          {/* RIGHT — Charts */}
          <div className="flex-1 px-4 sm:px-6 py-4 sm:py-5 flex flex-col gap-4 sm:gap-6 overflow-hidden">

            {/* Lucro chart */}
            <div>
              <h2 className="text-sm font-bold text-white mb-3">Estatísticas de negociações lucrativas</h2>
              <div className="bg-[#0E1620] rounded-xl border border-[#16202D] p-3">
                <AreaChart
                  data={profitData}
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="bg-[#0E1620] rounded-xl border border-[#16202D] p-4">
