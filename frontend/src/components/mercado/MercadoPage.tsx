...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center py-6 px-4 text-center">
      <ClipboardList size={36} className="text-[#1B2735] mb-3" />
      <p className="text-xs text-[#7E8DA2] leading-relaxed max-w-[200px]">
...
function MarketCard({ card }: { card: MarketCard }) {
  return (
    <div className="flex flex-col bg-[#0E1620] border border-[#16202D] rounded-2xl overflow-hidden">
      ...
      {/* Table */}
      <div className="flex-1 border-t border-[#16202D]">
        {/* Table header */}
        <div className="grid grid-cols-[1fr_80px_80px] gap-2 px-4 py-2">
          <span className="text-[9px] font-semibold text-[#7E8DA2] tracking-widest uppercase">Código Promocional</span>
          <span className="text-[9px] font-semibold text-[#7E8DA2] tracking-widest uppercase">Status</span>
          <span className="text-[9px] font-semibold text-[#7E8DA2] tracking-widest uppercase">Usando</span>
        </div>
        ...
        {card.codes.length === 0 ? (
          ...
          <div className="flex flex-col">
            {card.codes.map((c, i) => (
              <div key={i} className="grid grid-cols-[1fr_80px_80px] gap-2 px-4 py-2.5 border-t border-[#16202D]/60 items-center hover:bg-white/[0.02] transition-colors">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className={cn('text-[10px] font-medium', c.status === 'active' ? 'text-[#7E8DA2]' : 'text-[#7E8DA2]')}>
                    {c.date}
                  </span>
                </div>
                <div>
                  {c.status === 'active' ? (
                    ...
                    <span className="text-[10px] text-[#7E8DA2]">{c.label}</span>
                  )}
                </div>
              </div>
            ))}
      ...
      {/* Footer */}
      <div className="flex items-center justify-between gap-2 px-4 py-3 bg-[#0A101A] border-t border-[#16202D]">
        <button className="flex items-center gap-1.5 text-xs text-[#7E8DA2] hover:text-white transition-colors">
          <Clock size={12} />
          Mostrar todo o histórico
        </button>
        <div className="flex items-center gap-2">
          {card.extraButton && (
            <button className="px-3 py-1.5 rounded-lg bg-[#16202D] text-xs font-semibold text-white hover:bg-[#1B2735] transition-colors">
              {card.extraButton}
            </button>
          )}
...
export function MercadoPage() {
  return (
    <div className="flex-1 flex flex-col bg-[#0A101A] min-h-0 overflow-hidden">
      ...
      <div className="flex items-center px-6 border-b border-[#16202D] bg-[#0E1620] flex-shrink-0 gap-1">
