...
import { X, ChevronRight, ChevronLeft, Trophy, Radio, HelpCircle, Settings } from 'lucide-react'
...
interface MaisPanelProps {
  ...
  mobile?: boolean
  /** Mobile: abrir o painel de Configuracoes (em desktop ele fica no Sidebar) */
  onOpenConfig?: () => void
}
  ...
  return (
    <div className="relative w-7 h-7 flex-shrink-0">
      <img
        ...
        alt={flag}
        className="w-5 h-5 rounded-full object-cover border border-[#16202D] absolute top-0 left-0"
      />
...
function MiniFlag({ code1, code2 }: { code1: string; code2: string }) {
  return (
    <div className="relative w-7 h-5 flex-shrink-0">
      <img src={getImgSrc(code1)} className="w-5 h-5 rounded-full object-cover border border-[#16202D] absolute left-0 top-0 z-10" />
      <img src={getImgSrc(code2)} className="w-5 h-5 rounded-full object-cover border border-[#16202D] absolute left-2.5 top-0 z-0" />
    </div>
  )
}
...
function SinaisView({ onBack, onSelectAsset }: { onBack: () => void; onSelectAsset?: (asset: Asset) => void }) {
  ...
  return (
    <>
      {/* Header */}
      <div className="flex items-center gap-2 px-4 py-3 border-b border-[#16202D] flex-shrink-0">
        <button onClick={onBack} className="text-[#7E8DA2] hover:text-white transition-colors">
          <ChevronLeft size={16} />
        </button>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="flex flex-col">
          {SINAIS_ATIVOS.map((s, i) => (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 border-b border-[#16202D]/40 hover:bg-white/[0.04] transition-colors w-full text-left cursor-pointer"
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[10px] text-[#7E8DA2] mt-0.5">
                  Duração: <span className="text-white">{s.dur}</span>
                  <span className="ml-1.5 text-[#7E8DA2]">{s.time}</span>
                </div>
              </div>
              <DirCircle dir={s.dir} />
            </button>
        ...
        <div className="py-2 text-center">
          <span className="text-[9px] font-bold text-[#7E8DA2] tracking-widest">SINAIS PASSADOS</span>
        </div>

        {/* Past signals — not clickable, just history */}
        <div className="flex flex-col gap-2 px-3 pb-3">
          {SINAIS_PASSADOS.map((s, i) => (
            <div key={i} className="flex items-center gap-2 bg-[#101825] rounded-xl px-3 py-2.5 opacity-60">
              <MiniFlag code1={s.code1} code2={s.code2} />
              <div className="flex-1 min-w-0">
                <div className="text-xs font-semibold text-white truncate leading-tight">{s.name}</div>
                <div className="text-[10px] mt-0.5">
                  <span className="text-[#7E8DA2]">Duração: {s.dur}</span>
...
function TopView({ onBack }: { onBack: () => void }) {
  return (
    <>
      {/* Header */}
      <div className="flex items-start justify-between px-4 py-3 border-b border-[#16202D] flex-shrink-0">
        <button onClick={onBack} className="text-[#7E8DA2] hover:text-white transition-colors mt-0.5">
          <ChevronLeft size={16} />
        </button>
        <div className="flex-1 px-2">
          <h2 className="text-sm font-bold text-white leading-tight">Tabela de classificação</h2>
          <p className="text-[10px] text-[#7E8DA2] mt-0.5">do dia</p>
        </div>
        <button onClick={onBack} className="w-6 h-6 flex items-center justify-center rounded-full text-[#7E8DA2] hover:text-white hover:bg-white/10 transition-colors">
          <X size={13} />
        </button>
      </div>

      {/* My position */}
      <div className="px-4 py-3 border-b border-[#16202D] flex-shrink-0">
        ...
        {LEADERBOARD.map((entry) => {
          ...
          return (
            <div
              key={entry.pos}
              className={cn(
                'flex items-center gap-2 px-4 py-2.5 border-b border-[#16202D]/50 hover:bg-white/[0.02] transition-colors',
            ...
            >
              ...
              {medal ? (
                ...
                <span className="w-5 text-center text-[10px] text-[#7E8DA2] font-medium flex-shrink-0">{entry.pos}</span>
              )}
...
export function MaisPanel({ onClose, onSelectAsset, mobile = false, onOpenConfig }: MaisPanelProps) {
  const [view, setView] = useState<MaisView>('menu')

  return (
    <div
      className={cn(
        'flex flex-col bg-[#0E1620] flex-shrink-0',
        mobile ? 'w-full h-full' : 'border-r border-[#16202D]'
      )}
    ...
    >
      ...
      {view === 'top' ? (
        ...
        <>
          {/* Header */}
          <div className="flex items-center justify-between px-5 py-4 border-b border-[#16202D]">
          ...
          <div className="flex flex-col gap-1 p-3">
            {MENU_ITEMS.map((item) => (
              ...
              >
                ...
                <ChevronRight size={16} className="text-[#7E8DA2] group-hover:text-white transition-colors" />
              </button>
            ))}

            {/* Configuracoes — so aparece quando ha callback (mobile). Em desktop fica no Sidebar. */}
            {onOpenConfig && (
              <button
                onClick={() => { onOpenConfig(); onClose() }}
                className="flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-white/5 transition-colors text-left group"
              >
                <div className="w-9 h-9 rounded-xl border bg-[#7E8DA2]/10 border-[#7E8DA2]/20 flex items-center justify-center flex-shrink-0">
                  <Settings size={18} className="text-[#AEBBCB]" />
                </div>
